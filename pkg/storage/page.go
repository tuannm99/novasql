...
func GetCell(page *Page, index uint32) ([]byte, error) {
	...
	if location == 0 || size == 0 {
		return nil, NewStorageError(ErrCodeInvalidOperation, fmt.Sprintf("Cell at index %d has been deleted", index), nil)
	}
...
func (p *Page) Deserialize(data []byte) error {
	if len(data) != PageSize {
		return NewStorageError(ErrCodePageCorrupted, 
			fmt.Sprintf("Invalid page size: expected %d, got %d", PageSize, len(data)), nil)
	}
