...
func (db *Database) GetPage(pageID int) (*page.Page, error) {
	db.mu.RLock()
	defer db.mu.RUnlock()
	...
	page, err := db.engine.GetPage(pageID)
	if err != nil {
...
func (db *Database) WritePage(pageID int, data []byte) error {
	...
	if err := db.engine.WritePage(pageID, data); err != nil {
...
func (db *Database) PageCount() int {
	db.mu.RLock()
	defer db.mu.RUnlock()
	return db.engine.PageCount()
}

func (db *Database) PageSize() int {
	return db.engine.PageSize()
}
